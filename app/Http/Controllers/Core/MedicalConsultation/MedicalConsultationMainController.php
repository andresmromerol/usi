...
class MedicalConsultationMainController extends Controller
{
    ...
    function checkTimeDifference($oid, $feccan, $limitHour)
    {
        ...
        $record = DB::table('CMNCITMED')->select('CCMFECASI')->where('OID', $oid)->first();

        if ($record) {
            $startDate = Carbon::parse($record->CCMFECASI);
